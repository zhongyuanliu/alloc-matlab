    ...
    if (b_p == 0.0) {
      switch ((int)method) {
       ...
       case 2:
        ...
        if (Aeq->size[1] - 2 > solution->size[0]) {
          i1 = 1;
          i2 = 0;
        } else {
          i1 = Aeq->size[1] - 2;
        ...
        if ((loop_ub == 1) || (pipk == 1)) {
          ...
          c = c_solution->size[0];
          c_solution->size[0] = pipk;
          emxEnsureCapacity((emxArray__common *)c_solution, c, (int)sizeof
                            (double));
          for (c = 0; c < pipk; c++) {
            c_solution->data[c] = solution->data[c];
          }
          ...
          for (j = 0; j + 1 <= loop_ub; j++) {
            if (solution->data[j] != 0.0) {
              ia = c;
              for (pipk = 0; pipk < 3; pipk++) {
                ia++;
                d_expl_temp = y[pipk] + solution->data[j] * Aeq->data[ia % 3 +
                  Aeq->size[0] * (ia / 3)];
                y[pipk] = d_expl_temp;
              }
            }
        ...
        for (i0 = 0; i0 < loop_ub; i0++) {
          solution->data[ipiv->data[i0]] = r0->data[i0];
        }
