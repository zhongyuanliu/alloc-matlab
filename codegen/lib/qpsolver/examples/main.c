...
static void argInit_struct0_T(struct0_T *result)
{
  ...
  result->T_reserve = argInit_real_T();
  result->phi = argInit_real_T();
  result->phi_reserve = argInit_real_T();
  result->Tmax = argInit_real_T();
  result->Tmin = argInit_real_T();
  argInit_1x2_real_T(result->base);
...
static void main_qpsolver(void)
{
  ...
  init = argInit_boolean_T();

  /* Initialize function input argument 'thruster_data'. */
  argInit_1x8_struct0_T(thruster_data);
  ...
  qpsolver(init, thruster_data, &r1, argInit_real_T(), dv0, argInit_real_T(),
           argInit_real_T(), solution, alloc_out, &status);
