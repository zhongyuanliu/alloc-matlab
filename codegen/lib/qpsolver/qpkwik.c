...
static double KWIKfactor(const emxArray_real_T *Ac, const short iC_data[], short
  ...
  emxArray_real_T *b_H, short n);
static void ResetToColdStart(short m, short meq, short *nA, short iA_data[], int
  iA_size[1], short iC_data[], int iC_size[1]);
static void Unconstrained(const emxArray_real_T *Hinv, const emxArray_real_T
  *b_f, emxArray_real_T *x, short n);
  ...
  emxArray_real_T *b_H, short n)
{
  ...
  for (i22 = 0; i22 < 2; i22++) {
    ia = TL->size[0] * TL->size[1];
    TL->size[i22] = Linv->size[i22];
    emxEnsureCapacity((emxArray__common *)TL, ia, (int)sizeof(double));
  }
  ...
  for (i22 = 0; i22 < loop_ub; i22++) {
    for (ia = 0; ia < ar; ia++) {
      RLinv->data[ia + RLinv->size[0] * i22] = 0.0;
    }
  }
    ...
    if ((Linv->size[1] == 1) || (b->size[0] == 1)) {
      ...
      for (i22 = 0; i22 < ar; i22++) {
        ...
        for (ia = 0; ia < loop_ub; ia++) {
          C->data[i22] += Linv->data[i22 + Linv->size[0] * ia] * b->data[ia];
        }
      }
    } else {
      k = Linv->size[1];
      ...
      if (Linv->size[0] != 0) {
        ...
        while ((m > 0) && (ar <= 0)) {
          ar = -1;
          i22 = br + k;
          for (loop_ub = br; loop_ub + 1 <= i22; loop_ub++) {
  ...
  do {
    ...
    if (i <= nA) {
      ...
      for (i = 1; i <= n; i++) {
        for (j = i; j <= n; j++) {
          b_H->data[(i + b_H->size[0] * (j - 1)) - 1] = 0.0;
          for (b_k = (short)(nA + 1); b_k <= n; b_k++) {
            b_H->data[(i + b_H->size[0] * (j - 1)) - 1] -= TL->data[(i +
              TL->size[0] * (b_k - 1)) - 1] * TL->data[(j + TL->size[0] * (b_k -
              1)) - 1];
          }

          b_H->data[(j + b_H->size[0] * (i - 1)) - 1] = b_H->data[(i + b_H->
            size[0] * (j - 1)) - 1];
        }
      }
  ...
  *b_f, emxArray_real_T *x, short n)
{
  ...
  while (i <= n) {
    ...
    if ((a->size[1] == 1) || (b_f->size[0] == 1)) {
      y = 0.0;
      for (i21 = 0; i21 < a->size[1]; i21++) {
        y += a->data[a->size[0] * i21] * b_f->data[i21];
      }
    } else {
      y = 0.0;
      for (i21 = 0; i21 < a->size[1]; i21++) {
...
void qpkwik(const emxArray_real_T *Linv, const emxArray_real_T *Hinv, const
            emxArray_real_T *b_f, const emxArray_real_T *Ac, const
            emxArray_real_T *b, emxArray_int16_T *iA, short m, short n, short
            meq, emxArray_real_T *x, emxArray_real_T *lambda, double *status)
{
  int i20;
  int ar;
  emxArray_real_T *r;
  ...
  *status = 1.0;
  ...
  for (i20 = 0; i20 < 2; i20++) {
    ia = b_H->size[0] * b_H->size[1];
    b_H->size[i20] = Linv->size[i20];
    emxEnsureCapacity((emxArray__common *)b_H, ia, (int)sizeof(double));
  }
  ...
  if (nA > 0) {
    ...
    i20 = Rhs->size[0];
    Rhs->size[0] = b_f->size[0] + n;
    emxEnsureCapacity((emxArray__common *)Rhs, i20, (int)sizeof(double));
    ar = b_f->size[0];
    for (i20 = 0; i20 < ar; i20++) {
      Rhs->data[i20] = b_f->data[i20];
    }

    ar = n;
    for (i20 = 0; i20 < ar; i20++) {
      Rhs->data[i20 + b_f->size[0]] = 0.0;
    }
    ...
    do {
      ...
      if ((!DualFeasible) && (nA > 0) && (*status <= 100.0)) {
        Xnorm = KWIKfactor(Ac, iC_data, nA, Linv, RLinv, D, b_H, n);
        if (Xnorm < 0.0) {
          if (ColdReset) {
            *status = -2.0;
            exitg3 = 2;
            ...
            i20 = iA->size[0];
            iA->size[0] = iA_size[0];
            emxEnsureCapacity((emxArray__common *)iA, i20, (int)sizeof(short));
            ar = iA_size[0];
            for (i20 = 0; i20 < ar; i20++) {
              iA->data[i20] = iA_data[i20];
            }
          ...
          for (i = 1; i <= n; i++) {
            ...
            for (i20 = 0; i20 < b_n; i20++) {
              a->data[a->size[0] * i20] = b_H->data[(i + b_H->size[0] * i20) - 1];
            }
                ...
                if (Xnorm <= 0.0) {
                  *status = -2.0;
                  ...
                  emxEnsureCapacity((emxArray__common *)U, i20, (int)sizeof
                                    (double));
                  ar = b_H->size[0] * b_H->size[1];
                  for (i20 = 0; i20 < ar; i20++) {
                    U->data[i20] = -b_H->data[i20];
                  }
